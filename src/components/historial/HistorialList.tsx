// src/components/historial/HistorialList.tsx
  ...
  const renderCell = React.useCallback((item: HistorialMovimiento, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      ...
      case "entidad":
        ...
        // CORRECCIÓN: Verificar si el array tiene elementos
        if (entidadDesc.length > 0) {
            return <span className="text-xs">{entidadDesc.join(' ')}</span>;
        } else {
            return '-'; // Mostrar guion si no hay entidades relacionadas
        }
      case "detalles":
